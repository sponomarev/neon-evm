...
impl<'a> From<&'a evm::ExitReason> for Error {
    fn from(reason: &'a evm::ExitReason) -> Self {
        use evm::ExitError;
        use evm::ExitReason;
        match reason {
...
impl Tracer for ExecutiveTracer {
    ...
    fn done_trace_call(&mut self, gas_used: U256, res_output: &[u8]) {
        if self.skip_one {
            self.skip_one = false;
            return;
        }
    ...
    fn done_trace_create(&mut self, gas_used: U256, res_code: &[u8]) {
